...
//      Versione 0.0.1 del 30/03/2020
//        Autore : Alessandro Bramonti & Armando Brecciaroli
//
//      Indicatore Ichimoku multi timeframe
//      Mercato: tutti i timeframe - preferibilmente h4, Daily
//      Coppie: Tutte
//
//      ....
//      Sostituita la stringa che appariva nel menù a tendina "TimeFrameMq4" con "TimeFrame High (min)"
//      Sostituita la stringa "TimeFrameMq4" con "TimeFrame_High"
//      Sostituita la voce "MTF_Ichimoku_v1_Indicator" in public class con "A3_MTF_Ichimoku_Indicator"
//      Eliminato il riempimento "Histogram"
//      Sistemato il problema dello scostamento di Chikou e Kumo del TimeFrame superiore
//      Inserito valore di default per TimeFrame superiore: valore 1440 (Daily)
//
//      I valori da usare per i diversi timeframe sono espressi in minuti e sono i seguenti:
//      m1 = 1
//      m5 = 5
//      m15 = 15
//      h1 = 60
...
namespace cAlgo.Indicators
{
    ...
    public class A3_MTF_Ichimoku_Indicator : Indicator
    {
        Mq4Double Mq4Init()
        {
            ...
            SetIndexShift(5, Kijun * TimeFrame_High / Period() - 26);
            ...
            SetIndexShift(6, Kijun * TimeFrame_High / Period() - 26);

            SetIndexLabel(6, "Senkou Span B");
            SetIndexStyle(4, DRAW_LINE);
            SetIndexBuffer(4, Chikou_Buffer);
            SetIndexShift(4, -Kijun * TimeFrame_High / Period() + 26);
            SetIndexLabel(4, "Chikou Span");

            IndicatorShortName("A3_MTF_Ichimoku_Indicator");
            return 0;
            return 0;
        }
        Mq4Double Mq4Start()
        {
            Mq4Double chiko = 0;
            Mq4Double DnSenkou = 0;
            Mq4Double UpSenkou = 0;
            Mq4Double DnKijun = 0;
            Mq4Double UpKijun = 0;
            Mq4Double DnTenkan = 0;
            Mq4Double UpTenkan = 0;
            Mq4Double counted_bars = 0;
            Mq4Double y = 0;
            Mq4Double limit = 0;
            Mq4Double i = 0;
            Mq4DoubleArray TimeArray = new Mq4DoubleArray();

            y = 0;
            counted_bars = IndicatorCounted();

            ArrayCopySeries(TimeArray, MODE_TIME, Symbol.Code, TimeFrame_High);

            limit = Bars - counted_bars + TimeFrame_High / Period();

            for (i = 0,y = 0; i < limit; i++)
            {
                if (Time[i] < TimeArray[y])
                    y++;
                UpTenkan = iHigh(NULL, TimeFrame_High, iHighest(NULL, TimeFrame_High, MODE_HIGH, Tenkan, y));
                DnTenkan = iLow(NULL, TimeFrame_High, iLowest(NULL, TimeFrame_High, MODE_LOW, Tenkan, y));
                Tenkan_Buffer[i] = 0.5 * (UpTenkan + DnTenkan);

                UpKijun = iHigh(NULL, TimeFrame_High, iHighest(NULL, TimeFrame_High, MODE_HIGH, Kijun, y));
                DnKijun = iLow(NULL, TimeFrame_High, iLowest(NULL, TimeFrame_High, MODE_LOW, Kijun, y));
                Kijun_Buffer[i] = 0.5 * (UpKijun + DnKijun);

                UpSenkou = iHigh(NULL, TimeFrame_High, iHighest(NULL, TimeFrame_High, MODE_HIGH, Senkou, y));
                DnSenkou = iLow(NULL, TimeFrame_High, iLowest(NULL, TimeFrame_High, MODE_LOW, Senkou, y));

                SpanA_Buffer[i] = 0.5 * (Kijun_Buffer[i] + Tenkan_Buffer[i]);
                SpanB_Buffer[i] = 0.5 * (UpSenkou + DnSenkou);

                chiko = iClose(NULL, TimeFrame_High, y);
                if (chiko > 0)
                    Chikou_Buffer[i] = chiko;
        ...
        #region Indicatore Ichimoku Multi TimeFrame

        public const string NOME = "A3 MTF Ichimoku";
        //public const string VERSIONE = "0.0.1";

        [Parameter(NOME + " v.", Group = "Indicatore Ichimoku Multi TimeFrame", DefaultValue = "0.0.1")]
        ...
        [Parameter("Team A3", Group = "Developer", DefaultValue = "https://t.me/Obiriec")]
        ...
        [Parameter("TimeFrame Sup. (min)", Group = "Parametri", DefaultValue = 1440)]
        public int TimeFrame_High_parameter { get; set; }
        bool _TimeFrame_HighGot;
        Mq4Double TimeFrame_High_backfield;
        Mq4Double TimeFrame_High
        {
            get
            {
                if (!_TimeFrame_HighGot)
                    TimeFrame_High_backfield = TimeFrame_High_parameter;
                return TimeFrame_High_backfield;
            }
            set { TimeFrame_High_backfield = value; }
        }

        [Parameter("Tenkan", Group = "Parametri", DefaultValue = 9)]
        public int Tenkan_parameter { get; set; }
        bool _TenkanGot;
        Mq4Double Tenkan_backfield;
        Mq4Double Tenkan
        {
            get
            {
                if (!_TenkanGot)
                    Tenkan_backfield = Tenkan_parameter;
                return Tenkan_backfield;
            }
            set { Tenkan_backfield = value; }
        }

        [Parameter("Kijun", Group = "Parametri", DefaultValue = 26)]
        public int Kijun_parameter { get; set; }
        bool _KijunGot;
        Mq4Double Kijun_backfield;
        Mq4Double Kijun
        {
            get
            {
                if (!_KijunGot)
                    Kijun_backfield = Kijun_parameter;
                return Kijun_backfield;
            }
            set { Kijun_backfield = value; }
        }

        [Parameter("Senkou", Group = "Parametri", DefaultValue = 52)]
        public int Senkou_parameter { get; set; }
        bool _SenkouGot;
        Mq4Double Senkou_backfield;
        Mq4Double Senkou
        {
            get
            {
                if (!_SenkouGot)
                    Senkou_backfield = Senkou_parameter;
                return Senkou_backfield;
            }
            set { Senkou_backfield = value; }
        }

        #endregion

// ABre Colori delle linee
        [Output("Tenkan Sen", Color = Colors.SteelBlue, PlotType = PlotType.Line)]
        public IndicatorDataSeries Tenkan_Buffer_AlgoOutputDataSeries { get; set; }
        [Output("Kijun Sen", Color = Colors.Tomato, PlotType = PlotType.Line)]
        public IndicatorDataSeries Kijun_Buffer_AlgoOutputDataSeries { get; set; }
        public IndicatorDataSeries SpanA_Buffer_AlgoOutputDataSeries { get; set; }
        public IndicatorDataSeries SpanB_Buffer_AlgoOutputDataSeries { get; set; }
        [Output("Chikou Span", Color = Colors.PeachPuff, PlotType = PlotType.Line)]
        public IndicatorDataSeries Chikou_Buffer_AlgoOutputDataSeries { get; set; }
        [Output("SpanA2_Buffer", Color = Colors.PaleTurquoise, PlotType = PlotType.Line)]
        public IndicatorDataSeries SpanA2_Buffer_AlgoOutputDataSeries { get; set; }
        [Output("SpanB2_Buffer", Color = Colors.Plum, PlotType = PlotType.Line)]
        public IndicatorDataSeries SpanB2_Buffer_AlgoOutputDataSeries { get; set; }
        ...
        int indicator_color3 = SandyBrown;
        int indicator_color4 = Thistle;
        int indicator_color5 = Lime;
        int indicator_color6 = SandyBrown;
        int indicator_color7 = Thistle;
        ...
        Mq4Double indicator_width6 = 2;
        Mq4Double indicator_width7 = 2;
        Mq4Double indicator_width8 = 2;




        private Mq4OutputDataSeries Tenkan_Buffer;
        private Mq4OutputDataSeries Kijun_Buffer;
        private Mq4OutputDataSeries SpanA_Buffer;
        private Mq4OutputDataSeries SpanB_Buffer;
        private Mq4OutputDataSeries Chikou_Buffer;
        private Mq4OutputDataSeries SpanA2_Buffer;
        private Mq4OutputDataSeries SpanB2_Buffer;
        ...
        {
            if (Tenkan_Buffer_AlgoOutputDataSeries == null)
                Tenkan_Buffer_AlgoOutputDataSeries = CreateDataSeries();
            Tenkan_Buffer = new Mq4OutputDataSeries(this, Tenkan_Buffer_AlgoOutputDataSeries, ChartObjects, 0, 0, () => CreateDataSeries(), 2, Colors.Red);
            AllBuffers.Add(Tenkan_Buffer);
            if (Kijun_Buffer_AlgoOutputDataSeries == null)
                Kijun_Buffer_AlgoOutputDataSeries = CreateDataSeries();
            Kijun_Buffer = new Mq4OutputDataSeries(this, Kijun_Buffer_AlgoOutputDataSeries, ChartObjects, 0, 1, () => CreateDataSeries(), 2, Colors.Blue);
            AllBuffers.Add(Kijun_Buffer);
            if (SpanA_Buffer_AlgoOutputDataSeries == null)
                SpanA_Buffer_AlgoOutputDataSeries = CreateDataSeries();
            SpanA_Buffer = new Mq4OutputDataSeries(this, SpanA_Buffer_AlgoOutputDataSeries, ChartObjects, 2, 2, () => CreateDataSeries(), 1, Colors.SandyBrown);
            AllBuffers.Add(SpanA_Buffer);
            if (SpanB_Buffer_AlgoOutputDataSeries == null)
                SpanB_Buffer_AlgoOutputDataSeries = CreateDataSeries();
            SpanB_Buffer = new Mq4OutputDataSeries(this, SpanB_Buffer_AlgoOutputDataSeries, ChartObjects, 2, 3, () => CreateDataSeries(), 1, Colors.Thistle);
            AllBuffers.Add(SpanB_Buffer);
            if (Chikou_Buffer_AlgoOutputDataSeries == null)
                Chikou_Buffer_AlgoOutputDataSeries = CreateDataSeries();
            Chikou_Buffer = new Mq4OutputDataSeries(this, Chikou_Buffer_AlgoOutputDataSeries, ChartObjects, 0, 4, () => CreateDataSeries(), 1, Colors.Lime);
            AllBuffers.Add(Chikou_Buffer);
            if (SpanA2_Buffer_AlgoOutputDataSeries == null)
                SpanA2_Buffer_AlgoOutputDataSeries = CreateDataSeries();
            SpanA2_Buffer = new Mq4OutputDataSeries(this, SpanA2_Buffer_AlgoOutputDataSeries, ChartObjects, 0, 5, () => CreateDataSeries(), 1, Colors.SandyBrown);
            AllBuffers.Add(SpanA2_Buffer);
            if (SpanB2_Buffer_AlgoOutputDataSeries == null)
                SpanB2_Buffer_AlgoOutputDataSeries = CreateDataSeries();
            SpanB2_Buffer = new Mq4OutputDataSeries(this, SpanB2_Buffer_AlgoOutputDataSeries, ChartObjects, 0, 6, () => CreateDataSeries(), 1, Colors.Thistle);
            AllBuffers.Add(SpanB2_Buffer);

            AllOutputDataSeries.Add(Tenkan_Buffer_AlgoOutputDataSeries);
            AllOutputDataSeries.Add(Kijun_Buffer_AlgoOutputDataSeries);
            AllOutputDataSeries.Add(SpanA_Buffer_AlgoOutputDataSeries);
            AllOutputDataSeries.Add(SpanB_Buffer_AlgoOutputDataSeries);
            AllOutputDataSeries.Add(Chikou_Buffer_AlgoOutputDataSeries);
            AllOutputDataSeries.Add(SpanA2_Buffer_AlgoOutputDataSeries);
            AllOutputDataSeries.Add(SpanB2_Buffer_AlgoOutputDataSeries);
        ...
        public override void Calculate(int index)
        {
            ...
            {
                _currentIndex = index;
                Tenkan_Buffer.SetCurrentIndex(index);
                Kijun_Buffer.SetCurrentIndex(index);
                SpanA_Buffer.SetCurrentIndex(index);
                SpanB_Buffer.SetCurrentIndex(index);
                // ABra
                Chikou_Buffer.SetCurrentIndex(index - 26);
                // ABra
                SpanA2_Buffer.SetCurrentIndex(index + 26);
                // ABra
                SpanB2_Buffer.SetCurrentIndex(index + 26);
...
//{
        ...
        class Mq4OutputDataSeries : IMq4DoubleArray
        {
            ...
            private readonly A3_MTF_Ichimoku_Indicator _indicator;

            public Mq4OutputDataSeries(A3_MTF_Ichimoku_Indicator indicator, IndicatorDataSeries outputDataSeries, ChartObjects chartObjects, int style, int bufferIndex, Func<IndicatorDataSeries> dataSeriesFactory, int lineWidth, Colors? color = null)
            {
...
//int FILE_BIN = 8;
        ...
        abstract class Mq4Object : IDisposable
        {
            ...
            protected void DrawLine(string objectName, int index1, double y1, int index2, double y2, Colors color, double thickness = 1.0, cAlgo.API.LineStyle style = cAlgo.API.LineStyle.Dots)
            {
                _addedAlgoChartObjects.Add(objectName);
                _chartObjects.DrawLine(objectName, index1, y1, index2, y2, color, thickness, style);
            }

            protected void DrawLine(string objectName, DateTime date1, double y1, DateTime date2, double y2, Colors color, double thickness = 1.0, cAlgo.API.LineStyle style = cAlgo.API.LineStyle.Dots)
            {
                _addedAlgoChartObjects.Add(objectName);
                _chartObjects.DrawLine(objectName, date1, y1, date2, y2, color, thickness, style);
            }

            protected void DrawVerticalLine(string objectName, DateTime date, Colors color, double thickness = 1.0, cAlgo.API.LineStyle style = cAlgo.API.LineStyle.Dots)
            {
                _addedAlgoChartObjects.Add(objectName);
                _chartObjects.DrawVerticalLine(objectName, date, color, thickness, style);
            }

            protected void DrawVerticalLine(string objectName, int index, Colors color, double thickness = 1.0, cAlgo.API.LineStyle style = cAlgo.API.LineStyle.Dots)
            {
                _addedAlgoChartObjects.Add(objectName);
                _chartObjects.DrawVerticalLine(objectName, index, color, thickness, style);
            }

            protected void DrawHorizontalLine(string objectName, double y, Colors color, double thickness = 1.0, cAlgo.API.LineStyle style = cAlgo.API.LineStyle.Dots)
            {
                _addedAlgoChartObjects.Add(objectName);
            ...
            public override void Draw()
            {
                ...
                switch (ArrowCode)
                {
                    ...
                    case SYMBOL_LEFTPRICE:
                        ...
                        break;
                    default:
                        arrowString = A3_MTF_Ichimoku_Indicator.GetArrowByCode(ArrowCode);
